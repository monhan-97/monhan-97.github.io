...
对象字面量只能在计算属性语法中使用`Symbol`符号作为属性
...
- `Object.getOwnPropertyNames()`返回对象实例的**常规属性**数组 (包括**不可枚举属性**，但**不包括使用 Symbol 值作为名称的属性**）,
...
- `Object.keys()` 返回对象实例 **可枚举的字符串键属性**名组成数组 (但**不包括使用 Symbol 值作为名称的属性**)
